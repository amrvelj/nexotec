...
def test_create_customer_with_swiss_address_derives_canton(client):
    ...
    body = _create_customer(
        ...
        dealer_id,
        address={
            "street": "Bahnhofstrasse", "houseNumber": "1", "postalCode": "8001", "locality": "Zürich",
        },
    )
    assert body["address"]["canton"] == "ZH"
...
def test_create_customer_with_foreign_address_canton_is_null(client):
    ...
    body = _create_customer(
        ...
        dealer_id,
        address={
            "street": "Hauptstrasse", "houseNumber": "1", "postalCode": "10115", "locality": "Berlin",
            "country": "DE",
        },
    )
    assert body["address"]["canton"] is None
...
def test_create_customer_with_ambiguous_postal_code_canton_is_null(client):
    ...
    body = _create_customer(
        client,
        dealer_id,
        address={"street": "Rue de Lyon", "houseNumber": "1", "postalCode": "1290", "locality": "Versoix"},
    )
    assert body["address"]["canton"] is None


def test_update_customer_address_derives_canton(client):
    dealer_id = _create_dealer(client)
    ...
    response = client.patch(
        ...
        json={
            "address": {
                "street": "Marktgasse", "houseNumber": "10", "postalCode": "3011", "locality": "Bern",
            }
        },
        headers={**_bearer(token), "If-Match": "1"},
    )
    assert response.status_code == 200, response.text
    assert response.json()["address"]["canton"] == "BE"


def test_update_customer_clearing_address_clears_canton(client):
    dealer_id = _create_dealer(client)
    customer = _create_customer(
        client, dealer_id, address={"street": "Bahnhofstrasse", "houseNumber": "1", "postalCode": "8001", "locality": "Zürich"}
    )
